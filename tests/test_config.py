...
def test_empty_info():
    ...
    config_file = os.path.join(os.path.dirname(__file__), instance_config_path, "empty_instance.json")
...
def test_detectors_info():
    ...
    config_file = os.path.join(os.path.dirname(__file__), instance_config_path, "detector_instance.json")
    config_dict = json.load(open(config_file))
    instance = RedSunInstanceInfo(**config_dict)
    ...
    assert mocks[0].modelParams == {"paramInt": 1, "paramStr": "info", "paramFloat": 1.0}
    assert mocks[0].category == "area"
    assert mocks[0].exposureEGU == "ms"
    assert mocks[1].modelParams == {"paramInt": 2, "paramStr": "warning", "paramFloat": 2.0}
...
def test_motors_info():
    ...
    config_file = os.path.join(os.path.dirname(__file__), instance_config_path, "motor_instance.json")
    config_dict = json.load(open(config_file))
    instance = RedSunInstanceInfo(**config_dict)
